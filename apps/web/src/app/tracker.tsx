...
type Medication = { id: string; personId: string; name: string; inventoryItemId: string; stockNumerator: number; stockDenominator: number };
...
export default function Tracker() {
  ...
  const refresh = useCallback(async () => {
    ...
    const [data, occurrences] = await Promise.all([api<Workspace>(`/households/${householdId}/workspace`), api<Dose[]>(`/households/${householdId}/today?date=${date}`)]);
    setWorkspace(data); setDoses(occurrences);
  }, [householdId, date]);
  ...
  async function authenticate(event: FormEvent<HTMLFormElement>) {
    ...
    await run(async () => { await api(`/auth/${register ? 'register' : 'login'}`, { email: data.get('email'), password: data.get('password') }); setSession(await api<Session>('/auth/session')); });
  }
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    await run(async () => {
      ...
      if (panel === 'person') await api(`${base}/people`, { name: value('name') });
      if (panel === 'medication') { const [stockNumerator, stockDenominator] = parseQuantity(value('stock')); await api(`${base}/medications`, { personId: value('personId'), name: value('name'), form: 'tablet', stockNumerator, stockDenominator }); }
  ...
  function open(next: Panel, id = '') { setPanel(next); setSelected(id); setNotice(''); }
  ...
  return <main className="shell">
    <header><Link href="/" className="brand"><span className="brandMark">M</span>{t.title}</Link><div className="actions"><select aria-label={t.language} value={locale} onChange={e => { const lang = e.target.value as 'tr' | 'en'; setLocale(lang); localStorage.setItem('medication-locale', lang); }}><option value="tr">Türkçe</option><option value="en">English</option></select>{session && <button className="secondary" disabled={busy} onClick={() => run(async () => { await api('/auth/logout', {}); setSession(null); setWorkspace(empty); setDoses([]); })}>{t.logout}</button>}</div></header>
    {notice && <p role="status" className={`notice ${notice === 'saved' ? 'success' : ''}`}>{notice in t ? t[notice as keyof typeof t] : t.error}</p>}
    {loading ? <p className="empty">{t.loading}</p> : !session ? <section className="auth"><div><p className="eyebrow">{t.title}</p><h1>{t.intro}</h1><p>{t.safety}</p></div><form className="card" onSubmit={authenticate}><h2>{register ? t.register : t.login}</h2><label>{t.email}<input name="email" type="email" autoComplete="email" required maxLength={320} /></label><label>{t.password}<input name="password" type="password" autoComplete={register ? 'new-password' : 'current-password'} required minLength={12} maxLength={128} /></label><button disabled={busy}>{busy ? t.busy : register ? t.register : t.login}</button><button type="button" className="link" onClick={() => { setRegister(!register); setNotice(''); }}>{register ? t.login : t.register}</button></form></section> : <>
      <section className="heading"><div><p className="eyebrow">{t.intro}</p><h1>{t[tab]}</h1></div><div className="actions"><button onClick={() => open(workspace.people.length ? 'medication' : 'person')}>{workspace.people.length ? t.medication : t.person}</button><button className="secondary" disabled={busy} onClick={() => run(refresh)}>{t.refresh}</button></div></section>
      ...
      {panel && <section className="card editor"><form onSubmit={submit}><div className="sectionTitle"><h2>{t[panel]}</h2><button type="button" className="secondary" onClick={() => setPanel(null)}>{t.cancel}</button></div><div className="formGrid">
        {(panel === 'person' || panel === 'medication') && input('name')}
        {panel === 'medication' && <label>{t.personId}<select name="personId" required>{workspace.people.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label>}
        {(panel === 'medication' || panel === 'refill' || panel === 'count') && input('stock')}
        {panel === 'schedule' && <><label>{t.medicationId}<select name="medicationId" defaultValue={selected} required>{workspace.medications.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select></label>{input('dose')}{input('time', 'time', '09:00')}{input('start', 'date', today())}{input('end', 'date')}</>}
