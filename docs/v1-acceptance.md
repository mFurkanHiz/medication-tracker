...
| Treatment schedules | Daily, selected-weekday and interval-based schedules are supported, including exact times and named periods where applicable. | DONE | PR #9 adds Monday-based selected-weekday masks and effective-start-anchored N-day intervals across API, PostgreSQL persistence, due-dose validation, depletion forecast and TR/EN web management. Unit and PostgreSQL integration coverage includes DST-boundary dates, invalid rules and off-schedule administration rejection. |
