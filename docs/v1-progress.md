...
- 2026-09-16: PR #5 is merged. Package lending/return is implemented on PR #6 (`519ffc3`): permanent owner and current holder, explicit lend/return, immutable assignment/activity trail, idempotent household-authorized transitions, stock conservation with borrower use, and TR/EN web controls. PostgreSQL API and web/mobile/Docker CI run `35084942604` passed. The lending/return acceptance row is `DONE` for whole-package lending.
...
- Next action: merge PR #6 after the documentation checkpoint, then implement the `Treatment schedules` acceptance row: selected-weekday and arbitrary interval rules, date-aware dose generation, TR/EN web management, and DST/time-zone tests. Deploy the accumulated safe release and run production smoke as soon as project-scoped VPS SSH credentials are available.
