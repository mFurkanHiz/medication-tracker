...
| Medication lifecycle | Full medication create/read/update/soft-delete behavior preserves history and disables incompatible active relationships safely. | OPEN | Production review found incomplete CRUD/soft-delete; tracked as P0 correction. |
...
| Stock safety | A use event cannot create negative stock; insufficient stock is rejected atomically without partial administration/ledger writes. | OPEN | Confirmed P0 production defect. |
...
| Inventory counts | Bulk count/reconciliation exists and accepted count corrections are revisioned rather than rewriting history. | OPEN | Previously moved outside V1; original approved MVP requires it. |
...
| Schedule versioning | Editing a treatment schedule creates an effective-dated version; historical administration remains linked to the historical version. Soft-delete preserves history. | OPEN | Confirmed P0 gap. |
...
| Audit/history view | Medication, schedule, inventory/package and administration changes are visible in a coherent activity/history surface. | OPEN | Confirmed P0 gap; current UI exposes an incomplete history. |
